              ...
              >
                <div style={styles.capaFilme}>
                  ...
                  {hoveredFilmeId === item.filmeId && notasCache[item.filmeId]?.valorNota != null && (
                    ...
                    >
                      <span style={styles.overlayEstrelasLinha}>
                        {Array.from({ length: 5 }, (_, i) => (
                          <span key={i} style={{ color: i < notasCache[item.filmeId].valorNota ? '#f6c969' : 'rgba(255,255,255,0.34)' }}>★</span>
                        ))}
                      </span>
                      <span style={styles.overlayEstrelasTexto}>Ver resenha</span>
                    </button>
                  )}
                </div>
...
const styles = {
  ...
  overlayEstrelas: {
    ...
    left: 0,
    right: 0,
    background: 'linear-gradient(transparent 0%, rgba(0,0,0,0.52) 24%, rgba(0,0,0,0.86) 100%)',
    display: 'flex',
    flexDirection: 'column',
    alignItems: 'center',
    justifyContent: 'center',
