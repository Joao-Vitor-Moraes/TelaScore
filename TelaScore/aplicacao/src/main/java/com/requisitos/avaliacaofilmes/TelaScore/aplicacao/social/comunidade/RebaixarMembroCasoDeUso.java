...
public class RebaixarMembroCasoDeUso {
    private final ComunidadeRepositorio repositorio;

    public RebaixarMembroCasoDeUso(ComunidadeRepositorio repositorio) {
    ...
    public void executar(int comunidadeId, int usuarioId, int solicitanteId) {
        ...
        if (!ehCriador) {
            throw new SecurityException("Apenas o criador da comunidade pode rebaixar moderadores.");
        }

        repositorio.atualizarPapelMembro(
                cid,
                new UsuarioId(usuarioId),
                PapelComunidade.MEMBRO
        );
    }
}